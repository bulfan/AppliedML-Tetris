#All of your Tetris "front end" (rendering, keyboard input, etc.)	
# • Keep it purely about visualization / human-in-the-loop control.
#• Consider splitting out an environment wrapper (e.g. tetris_env.py) 
# if you want to run headless RL.
...
TETRIS_AI = None
# ^^ uncomment if you want manual play
...
COLORS = {
    ...
    'pieces': {
        0: QColor(20, 20, 30),      # Empty
        1: QColor(255, 100, 100),   # I-piece - Red
        2: QColor(100, 255, 100),   # O-piece - Green  
        3: QColor(100, 100, 255),   # T-piece - Blue
        4: QColor(255, 255, 100),   # S-piece - Yellow
        5: QColor(255, 100, 255),   # Z-piece - Magenta
        6: QColor(100, 255, 255),   # J-piece - Cyan
        7: QColor(255, 150, 100),   # L-piece - Orange
    }
}
...
class Tetris(QMainWindow):
    def __init__(self):
        ...
        self.ai_mode = False  # Manual play by default
        self.ai_speed = 300   # AI move speed
    ...
    def pause(self):
        ...
        if self.isPaused:
            self.timer.stop()
            self.tboard.msg2Statusbar.emit("⏸️ PAUSED - Press P to resume")
        else:
            self.timer.start(self.speed, self)
            self.updateStatusBar()
    ...
    def timerEvent(self, event):
        if event.timerId() == self.timer.timerId():
            ...
            if lines > 0:
                ...
                self.tboard.score += lines * 100  # Bonus for clearing lines
                
            self.tboard.score += 1  # Small score for each step
    ...
    def executeAction(self, action):
        ...
        if action == 0:  # Move left
            ...
            self.tboard.score += dropped_lines * 2  # Bonus for hard drop
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_P:
            self.pause()
            return
        elif key == Qt.Key_A:  # Toggle AI mode
            self.toggleAIMode()
            return
        ...
        # Manual controls (only work when not in AI mode)
        if not self.ai_mode:
            if key == Qt.Key_Left:
                BOARD_DATA.moveLeft()
...
class SidePanel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        y_offset += 80  # Space for the buttons
...
class Board(QFrame):
    ...
    def __init__(self, parent, gridSize):
        ...
        self.gridSize = gridSize
        self.tetris_window = parent  # Store reference to main window
        self.setStyleSheet("""
            QFrame {
                background-color: rgb(20, 20, 30);
...
def drawSquare(painter, x, y, val, s, is_current=False):
    ...
    if val != 0:
        ...
        painter.drawLine(x + 2, y + 2, x + s - 3, y + 2)  # Top highlight
        painter.drawLine(x + 2, y + 2, x + 2, y + s - 3)  # Left highlight
