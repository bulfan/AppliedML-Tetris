...
class TetrisTrainer:
    ...
    def __init__(self, agent_name="dqn_advanced"):
        ...
        # Training parameters
        self.total_episodes = 2000  # Proper training episodes
        self.eval_frequency = 100   # Evaluate every N episodes
        self.save_frequency = 200   # Save model every N episodes
        self.max_steps_per_episode = 2000  # Increased for proper games
    ...
    def train_agent(self):
        ...
        if not self.agent_name.startswith("dqn"):
            print(f"⚠️  {self.agent_name} is not a trainable agent")
            return
        ...
        print(f"⏱️  Total training time: {datetime.now() - start_time}")
    ...
    def _run_episode(self, episode):
        ...
        while not self.env.game_over and steps < self.max_steps_per_episode:
            # Get action from AI manager (with training=True for exploration)
    ...
    def _evaluate_agent(self, episode, num_games=5):
        ...
        for game in range(num_games):
            ...
            while not self.env.game_over and steps < self.max_steps_per_episode:
                # Get action without training (no exploration)
